...
- When the change must be tested through the installed `mm` command, also run `cargo build --release --workspace` and update the installed binary with `install -m 755 target/release/mm "$HOME/.local/bin/mm"`. A debug build only updates `target/debug/mm`.
