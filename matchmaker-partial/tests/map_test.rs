...
fn test_indexmap_set_path() {
    ...
    assert!(
        res.is_ok(),
        "Setting indexmap key via path should succeed, got {:?}",
        res
    );
    ...
    assert!(
        res2.is_ok(),
        "Setting indexmap field directly should succeed, got {:?}",
        res2
    );
    assert_eq!(p2.binds.get("ctrl-q"), Some(&"Exit".to_string()));
}
...
fn test_indexset_apply() {
    ...
    let mut s = SetStruct::default();
    s.tags.insert("first".into());

    let mut p = PartialSetStruct::default();
    p.tags.insert("second".into());

    s.apply(p);
    assert!(s.tags.contains("first"));
    assert!(s.tags.contains("second"));
}

#[test]
fn test_indexmap_apply_overwrite_vs_extend() {
    #[partial]
    #[derive(Debug, PartialEq, Default, Clone)]
    struct Val {
        pub x: i32,
